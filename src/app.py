...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Nạp Action + Observation vào lịch sử hội thoại để LLM tiếp tục suy luận ở vòng kế tiếp
            messages.append({
                "role": "assistant",
                "type": "tool_call",
                "tool_name": tool_name,
        ...
        fallback_answer = "Đã đạt giới hạn số bước suy luận cho phép (MAX_ITERATIONS) mà chưa tổng hợp được câu trả lời cuối cùng."
        print(f"⚠️ [CHÚ Ý]: {fallback_answer}")
        trace_logs.append({
            "step": step,
            "query": user_query,
            "action_type": "FINAL_ANSWER",
...
if __name__ == "__main__":
    ...
    if "--interactive" in sys.argv:
        ...
        print("   - Câu hỏi chung: 'Làm sao để quản lý công việc hiệu quả hơn?'")
        print("   - Tra cứu công việc: 'Hãy cho tôi xem danh sách công việc đang chờ xử lý'")
        print("   - Tạo công việc mới: 'Tạo công việc Ôn thi cuối kỳ, hạn 25/09/2026, ưu tiên cao'")
